...
def aggregate_watersheds(watersheds_fc, lake_id_field, nhd_gdb, pour_dir,
    ...
    if mode == 'interlake':
        ...
        arcpy.SelectLayerByLocation('junctions', 'INTERSECT', 'tenha_lakes', search_distance = '1m')
            ...
            if count_jxns == 0:
                ...
                if mode == 'interlake':
                    arcpy.TraceGeometricNetwork_management(hydro_net, "upstream",
                                    'this_lake_jxns', "TRACE_UPSTREAM", in_barriers = tenha_junctions)
...
##
##
def aggregate_watersheds(nhd_gdb, watersheds, topoutfolder, filterlakes,
    ...
    # Naming Convention
    subregion_number = os.path.basename(nhd_gdb)
    ...
    # NHD variables:
    flowline = os.path.join(nhd_gdb, "Hydrography", "NHDFlowline")
    waterbody = os.path.join(nhd_gdb, "Hydrography", "NHDWaterbody")
    network = os.path.join(nhd_gdb, "Hydrography", "HYDRO_NET")
    junction = os.path.join(nhd_gdb, "Hydrography", "HYDRO_NET_Junctions")
