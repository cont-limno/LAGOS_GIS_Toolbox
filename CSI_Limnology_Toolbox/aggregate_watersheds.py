...
def cumulative_watersheds(nhd_gdb, watersheds_fc, lake_id_field, lakes_output,
                            agg_watersheds_output):
    """Creates a feature class with all the aggregated upstream watersheds for all
    eligible lakes (>4ha and certain FCodes) in this subregion. Also copies the
    4 hectare lakes of certain type to an output."""
    # if the watersheds are in another projection, change them to NAD83 same as
    # the NHD
    ...
    cu.multi_msg("Selecting eligible lakes and saving to output.")
    arcpy.Select_analysis(nhd_waterbody, lakes_output, where_clause)
    ...
    # skip isolated lakes
    arcpy.MakeFeatureLayer_management(lakes_output, 'all_lakes')
    arcpy.SelectLayerByLocation_management('all_lakes', 'INTERSECT',
                                        'junctions')
    ...
    with arcpy.da.SearchCursor('eligible_lakes', [lake_id_field]) as cursor:
        for row in cursor:
            ...
            arcpy.Dissolve_management("watersheds", "dissolve")
##                                    lake_id_field)
            arcpy.Erase_analysis('dissolve', 'this_lake',
                                'erased')
            if not arcpy.Exists(agg_watersheds_output):
                arcpy.CopyFeatures_management('erased', agg_watersheds_output)
            else:
                arcpy.Append_management('erased', agg_watersheds_output)

            for item in ['this_lake', 'this_lake_jxns', 'upstream', 'dissolve',
                        'erased']:
                arcpy.Delete_management(item)
    arcpy.ResetEnvironments()

def test():
    nhd_gdb = r'E:\RawNHD_byHUC\NHDH0109.gdb'
    watersheds_fc = r'C:\GISData\CSI_Watershed_Delineations.gdb\Clean_Watersheds\CleanWatersheds_0109'
    lake_id_field = 'Permanent_Identifier'
    lakes_output = 'C:/GISData/Scratch/Scratch.gdb/test_aggws_LAKES'
    agg_watersheds_output = 'C:/GISData/Scratch/Scratch.gdb/test_aggws_SHEDS'
    cumulative_watersheds(nhd_gdb, watersheds_fc, lake_id_field, lakes_output,
                            agg_watersheds_output)
...
##
##        try:
##            # Trace the network upstream from the junctions from above, stopping at ten hectare junctions if interlake mode
##            if aggregate_method == 'interlake':
